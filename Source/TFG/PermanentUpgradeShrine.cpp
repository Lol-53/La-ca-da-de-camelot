...
APermanentUpgradeShrine::APermanentUpgradeShrine() {
  ...
  static ConstructorHelpers::FObjectFinder<UStaticMesh> ShrineMeshAsset(
      TEXT("/Game/MedievalDungeon/Meshes/Props/"
           "SM_Gargoyle_Statue_On_Stand.SM_Gargoyle_Statue_On_Stand"));
  if (ShrineMeshAsset.Succeeded()) {
    ShrineMesh->SetStaticMesh(ShrineMeshAsset.Object);
  }

  LightParticles =
      CreateDefaultSubobject<UParticleSystemComponent>(TEXT("LightParticles"));
  ...
  GoldenLight =
      CreateDefaultSubobject<UPointLightComponent>(TEXT("GoldenLight"));
  ...
  InteractionSphere->OnComponentBeginOverlap.AddDynamic(
      this, &APermanentUpgradeShrine::OnInteractionBegin);
  InteractionSphere->OnComponentEndOverlap.AddDynamic(
      this, &APermanentUpgradeShrine::OnInteractionEnd);
  ...
  InteractionPrompt->SetText(
      FText::FromString(TEXT("E - MEJORAS PERMANENTES")));
...
void APermanentUpgradeShrine::BeginPlay() {
  ...
  UE_LOG(LogPermanentUpgradeShrine, Display,
         TEXT("[FUENTE DE LUZ] Santuario preparado en %s."),
...
void APermanentUpgradeShrine::Tick(const float DeltaSeconds) {
  ...
  if (ACharacter *Player = UGameplayStatics::GetPlayerCharacter(this, 0)) {
    ...
    if (bIsNear != bPlayerNearby) {
      ...
      UE_LOG(LogPermanentUpgradeShrine, Display,
             TEXT("[FUENTE DE LUZ] Jugador %s del alcance."),
...
void APermanentUpgradeShrine::OnInteractionBegin(
  ...
  if (OtherActor == UGameplayStatics::GetPlayerCharacter(this, 0)) {
    ...
    UE_LOG(LogPermanentUpgradeShrine, Display,
           TEXT("[FUENTE DE LUZ] Jugador dentro del alcance."));
  }
}
...
void APermanentUpgradeShrine::OpenUpgradeMenu() {
  ...
  if (PlayerController) {
    ...
    if (UpgradeWidget) {
      UpgradeWidget->OnMenuClosed.AddDynamic(
          this, &APermanentUpgradeShrine::CloseUpgradeMenu);
      ...
      InteractionPrompt->SetVisibility(false);
      UE_LOG(LogPermanentUpgradeShrine, Display,
             TEXT("[FUENTE DE LUZ] Menu abierto."));
    }
  }
}
...
void APermanentUpgradeShrine::CloseUpgradeMenu() {
  ...
  UE_LOG(LogPermanentUpgradeShrine, Display,
         TEXT("[FUENTE DE LUZ] Menu cerrado."));
}
...
void APermanentUpgradeShrine::SnapToFloor() {
  ...
  FCollisionQueryParams Params(SCENE_QUERY_STAT(PermanentUpgradeShrineFloor),
                               false, this);
