...
AInteractiveNPCBase::AInteractiveNPCBase() {
  ...
  InteractionRange =
      CreateDefaultSubobject<USphereComponent>(TEXT("InteractionRange"));
  ...
  InteractionRange->OnComponentBeginOverlap.AddDynamic(
      this, &AInteractiveNPCBase::OnInteractionRangeBegin);
  InteractionRange->OnComponentEndOverlap.AddDynamic(
      this, &AInteractiveNPCBase::OnInteractionRangeEnd);
  ...
  InteractionPrompt->SetVerticalAlignment(
...
void AInteractiveNPCBase::BeginPlay() {
  ...
  UE_LOG(LogNPCInteraction, Display,
         TEXT("[NPC] %s (%s) usara la interaccion unica de BP_LineTrace para "
              "avanzar con E."),
         *GetName(), *NombreNPC.ToString());
}
...
void AInteractiveNPCBase::Tick(const float DeltaSeconds) {
  ...
  if (DialogueIndex != LastObservedDialogueIndex) {
    ...
    UE_LOG(LogNPCInteraction, Display,
           TEXT("[NPC] %s avanzo dialogo: %d -> %d."), *GetName(),
...
void AInteractiveNPCBase::OnInteractionRangeBegin(
  ...
  UE_LOG(LogNPCInteraction, Display,
         TEXT("[NPC] Jugador cerca de %s: E-Interactuar."), *GetName());
}
...
void AInteractiveNPCBase::OnInteractionRangeEnd(
  ...
  UE_LOG(LogNPCInteraction, Display,
         TEXT("[NPC] Jugador fuera del alcance de %s."), *GetName());
}
...
void AInteractiveNPCBase::HandleInteractPressed() {
  if (bPlayerInRange && NearbyPlayer.IsValid()) {
    ...
    if (InteractionFunction) {
      ...
      UE_LOG(LogNPCInteraction, Display,
             TEXT("[NPC] E pulsada en %s. Indice de dialogo: %d."), *GetName(),
             DialogueIndex);
    } else {
      UE_LOG(LogNPCInteraction, Warning,
             TEXT("[NPC] %s no implementa Interactuar."), *GetName());
    }
  }
}
...
void AInteractiveNPCBase::UpdatePromptVisibility() {
  const bool bShouldShow =
      bPlayerInRange && NearbyPlayer.IsValid() && ReadDialogueIndex() == 0;
...
void AInteractiveNPCBase::FacePromptToCamera() const {
  ...
  FRotator LookRotation = (CameraManager->GetCameraLocation() -
...
int32 AInteractiveNPCBase::ReadDialogueIndex() const {
  ...
  if (IndexProperty) {
    ...
    DialogueIndex = static_cast<int32>(
        IndexProperty->GetSignedIntPropertyValue(ValueAddress));
  }
...
void AInteractiveNPCBase::SelectDialoguePackage() {
  ...
  const bool bHasValidPackages = !ValidPackageIndices.IsEmpty();
  if (!bHasValidPackages) {
    UE_LOG(LogNPCInteraction, Display,
           TEXT("[NPC] %s no tiene paquetes configurados; se conserva el "
                "Dialogo heredado."),
           *NombreNPC.ToString());
  } else {
    ...
    FStrProperty *DialogueLineProperty =
        DialogueProperty ? CastField<FStrProperty>(DialogueProperty->Inner)
                         : nullptr;
    FTextProperty *DialogueTextProperty =
        DialogueProperty ? CastField<FTextProperty>(DialogueProperty->Inner)
                         : nullptr;
    if (!DialogueProperty || (!DialogueLineProperty && !DialogueTextProperty)) {
      UE_LOG(LogNPCInteraction, Error,
             TEXT("[NPC] %s no puede aplicar el paquete porque Dialogo no es "
                  "un array de Text o String."),
             *NombreNPC.ToString());
    } else {

      void *DialogueAddress =
          DialogueProperty->ContainerPtrToValuePtr<void>(this);
      ...
      for (int32 LineIndex = 0; LineIndex < SelectedPackage.Lineas.Num();
           ++LineIndex) {
        ...
        if (DialogueTextProperty) {
          ...
          DialogueLineProperty->SetPropertyValue(
              DialogueLineAddress, SelectedPackage.Lineas[LineIndex]);
        }
      }

      UE_LOG(LogNPCInteraction, Display,
             TEXT("[NPC] %s ha elegido el paquete '%s' (%d frases de %d "
                  "paquetes disponibles)."),
             *NombreNPC.ToString(), *SelectedPackage.NombrePaquete.ToString(),
             SelectedPackage.Lineas.Num(), ValidPackageIndices.Num());
    }
  }
}
...
void AInteractiveNPCBase::NotifyDialogueState(const bool bActive) {
  ...
  if (bActive) {
    if (!DialogueNameWidget) {
      if (APlayerController *OwningController =
        ...
        DialogueNameWidget = CreateWidget<UNPCDialogueNameWidget>(
            OwningController, UNPCDialogueNameWidget::StaticClass());
        if (DialogueNameWidget) {
    ...
    UE_LOG(LogNPCInteraction, Display, TEXT("[NPC] Dialogo iniciado con %s."),
           *NombreNPC.ToString());
  } else {
    if (DialogueNameWidget) {
    ...
    UE_LOG(LogNPCInteraction, Display, TEXT("[NPC] Dialogo finalizado con %s."),
           *NombreNPC.ToString());
  }
}
