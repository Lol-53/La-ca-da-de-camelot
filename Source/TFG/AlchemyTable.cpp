...
AAlchemyTable::AAlchemyTable()
{
	...
	static ConstructorHelpers::FObjectFinder<UStaticMesh> HealthPotAsset(
		TEXT("/Game/MedievalDungeon/Meshes/Props/SM_Pot_A_Complete.SM_Pot_A_Complete"));
	static ConstructorHelpers::FObjectFinder<UStaticMesh> ManaPotAsset(
		TEXT("/Game/MedievalDungeon/Meshes/Props/SM_Pot_B_Complete.SM_Pot_B_Complete"));
	static ConstructorHelpers::FObjectFinder<UStaticMesh> EnergyPotAsset(
		TEXT("/Game/MedievalDungeon/Meshes/Props/SM_Pot_C_Complete.SM_Pot_C_Complete"));
	...
	if (HealthPotAsset.Succeeded()) HealthPot->SetStaticMesh(HealthPotAsset.Object);
	...
	if (ManaPotAsset.Succeeded()) ManaPot->SetStaticMesh(ManaPotAsset.Object);
	...
	if (EnergyPotAsset.Succeeded()) EnergyPot->SetStaticMesh(EnergyPotAsset.Object);

	HealthLight = CreateDefaultSubobject<UPointLightComponent>(TEXT("HealthLight"));
	...
	EnergyLight = CreateDefaultSubobject<UPointLightComponent>(TEXT("EnergyLight"));
...
void AAlchemyTable::Tick(const float DeltaSeconds)
{
	...
	if (Player)
	{
		const float InteractionDistance = InteractionSphere->GetScaledSphereRadius() + 70.0f;
		...
		if (bIsNear != bPlayerNearby)
		{
			...
			UE_LOG(LogAlchemyTable, Display, TEXT("[ALQUIMIA] Jugador %s del alcance."),
	...
	if (APlayerController* PlayerController = UGameplayStatics::GetPlayerController(this, 0))
	{
		...
		if (APlayerCameraManager* Camera = PlayerController->PlayerCameraManager)
		{
			InteractionPrompt->SetWorldRotation(
				(Camera->GetCameraLocation() - InteractionPrompt->GetComponentLocation()).Rotation());
		}
	}
}
...
void AAlchemyTable::OpenCraftingMenu()
{
	...
	CraftingWidget->OnMenuClosed.AddDynamic(this, &AAlchemyTable::CloseCraftingMenu);
	...
	bMenuOpen = true;
	InteractionPrompt->SetVisibility(false);
	UE_LOG(LogAlchemyTable, Display, TEXT("[ALQUIMIA] Menu de fabricacion abierto."));
}
...
void AAlchemyTable::CloseCraftingMenu()
{
	...
	UE_LOG(LogAlchemyTable, Display, TEXT("[ALQUIMIA] Menu de fabricacion cerrado."));
}
...
void AAlchemyTable::SnapToFloor()
{
	...
	FCollisionQueryParams Params(SCENE_QUERY_STAT(AlchemyTableFloor), false, this);
